...
- [BREAKING] The `ConstantFeePolicy` now stores the root of its lookup-key procedure in the new `lookup_key_proc_root` storage slot and invokes it dynamically via `dyncall` (defaulting to the built-in `build_note_fee_lookup_key`, which keys on the note's script root), and its Rust fee schedule is keyed by the new `NoteFeeLookupKey` type instead of `NoteScriptRoot` ([#3352](https://github.com/0xMiden/protocol/pull/3352)).
