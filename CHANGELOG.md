...
- [BREAKING] `AuthNetworkAccount::new` now allowlists the standardized `FeeSponsorshipNote` script root alongside the `NetworkAccountConfigNote` one, exposed together as `AuthNetworkAccount::default_note_script_roots`, and `BasicConstantFeePolicy::new` schedules that set at an explicit 0 fee. This also removes the fee-schedule blocker that made the force-allowlisted config note unconsumable on accounts whose policy never scheduled it, unfreezing the AggLayer bridge's allowlists. Note this does not on its own make the AggLayer faucet reconfigurable: it is owner-gated to the bridge account, which has no procedure that emits a config note. Both the allowlist and the fee schedule are storage, so accounts built from the same seed now have a different ID, and accounts pricing everything at 0 now allowlist a sponsorship root that can only ever abort ([#3401](https://github.com/0xMiden/protocol/issues/3401)) ([#3397](https://github.com/0xMiden/protocol/issues/3397)).
