...
- [BREAKING] Replaced `StandardNote::expected_num_storage_items` with `StandardNote::accepts_num_storage_items`, which reports every storage size a note kind accepts instead of a single value that was neither exact nor a bound for MINT and the config notes ([#XXXX](https://github.com/0xMiden/protocol/pull/XXXX)).
...
- [BREAKING] Fixed `TokenSymbol::try_from(Felt)` to reject values below `MIN_ENCODED_VALUE`. Implemented `Display` for `TokenSymbol`, replacing the fallible `to_string()` method. Removed the `Default` derive ([#2464](https://github.com/0xMiden/protocol/issues/2464)).
