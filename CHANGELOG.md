...
- [BREAKING] Renamed the component management notes to use "config" terminology: `FaucetPolicyActionNote` (`FaucetPolicyAction`) is now `FaucetPolicyConfigNote` (`FaucetPolicyConfig`), `OwnerActionNote` (`OwnerAction`) is now `OwnerConfigNote` (`OwnerConfig`), `PauseActionNote` (`PauseAction`) is now `PauseConfigNote` (`PauseConfig`), and `RbacActionNote` (`RbacAction`) is now `RbacConfigNote` (`RbacConfig`). Their note script modules (`miden::standards::notes::<name>_config`), `StandardNote` variants, `<NOTE>_CONSUMPTION_CYCLES` constants and MASM error constants follow suit, and every config note builder - including the already-named `NetworkAccountConfigNote` - takes the carried action via `.config()` instead of `.action()` ([#3434](https://github.com/0xMiden/protocol/pull/3434)).
- [BREAKING] Made `RbacConfigNote` and `OwnerConfigNote` create their `NetworkAccountTarget` attachment implicitly from the managed account, since both are network notes. Their builders no longer accept attachments, return `NetworkAccountTargetError` when the managed account is not public, and the notes convert into an `AccountTargetNetworkNote` via `From` ([#3434](https://github.com/0xMiden/protocol/pull/3434)).
