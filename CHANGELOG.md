...
- Moved the note ID helpers into the protocol library as `miden::protocol::input_note::find_note` and `miden::protocol::output_note::compute_note_id`. The new `INPUT_NOTE_INDEX_REQUEST_EVENT` lets transaction hosts provide an input-note index hint: successful lookups authenticate it against the `NoteId` cached by the transaction prologue, while reported misses are validated by a full scan. This makes the common found case O(1) in the VM while preserving soundness, and provides a shared fast lookup for `FeeSponsorshipNote` and the fee-collection work tracked in [#3340](https://github.com/0xMiden/protocol/issues/3340) ([#3424](https://github.com/0xMiden/protocol/pull/3424)).
