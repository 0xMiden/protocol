...
- [BREAKING] AggLayer bridge and faucet accounts now take their fee policy manager at deployment, through the new `AggLayerBridge::account_builder` / `AggLayerFaucet::account_builder` or the existing constructors; `NetworkNotePricer::agglayer_bridge_fee_policy_manager` / `agglayer_faucet_fee_policy_manager` build one that prices every note those accounts consume from its benchmarked cost ([#3487](https://github.com/0xMiden/protocol/issues/3487)).
- [BREAKING] AggLayer bridge and faucet accounts install the `ConstantFeeManager` and allowlist `CONSTANT_FEE_POLICY_CONFIG` notes, so their `ADMIN` role can reprice the deployed fee schedule; the faucet moved from `Authority::OwnerControlled` to `Authority::RbacControlled` and its constructors now take a deployment admin, leaving minting gated on the bridge as its `Ownable2Step` owner ([#3487](https://github.com/0xMiden/protocol/issues/3487)).
