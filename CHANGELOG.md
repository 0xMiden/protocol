...
- [BREAKING] Flipped `AuthSingleSigAcl` ACL to an exempt list: every called procedure now requires a signature unless its root is in `exempt_procedures`. Removed `allow_unauthorized_{output,input}_notes` and the `config` storage slot; renamed trigger-roots map to `exempt_procedure_roots`. See type doc for full logic. Part of [#2964](https://github.com/0xMiden/protocol/issues/2964).
