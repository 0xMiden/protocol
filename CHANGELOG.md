...
### Changes
- [BREAKING] Transaction fees are now paid by the authentication procedure creating a public BATCH_FEE note funded from the account's vault, before the transaction summary is created so the note is covered by the signature. Adds the composable `miden::standards::auth::fee::pay_fee` MASM procedure (native fee asset by default; a different asset via an auth-args commitment to conversion info), per-scheme cycle estimates (`signature::estimate_authentication_cycles`), the `tx::get_fee_faucet_id` kernel accessor, and Rust helpers `BatchFeeNote::derive_serial_number` and `FeeConversionInfo`. The standard singlesig component pays fees automatically; on zero-base-fee chains no note is created ([#2899](https://github.com/0xMiden/protocol/discussions/2899)).
...
- [BREAKING] Added the BATCH_FEE standard note (`BatchFeeNote` / `miden::standards::notes::batch_fee`): a public, untargeted P2ID-like note consumable by any account, intended for paying transaction fees to a batch builder. Includes MASM `batch_fee::prepare_note` / `batch_fee::create_output_note` creation procedures, the unique `0xFEE` note tag, and a `StandardNote::BATCH_FEE` variant (breaking for exhaustive matches on `StandardNote`) ([#3117](https://github.com/0xMiden/protocol/issues/3117)).
