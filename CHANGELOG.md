...
### Changes
- Added the `FeeSponsorshipNote` standard note and a `miden::standards::note::note_id` MASM module for computing a note's ID on-chain. The sponsorship note carries no attachments: its note storage binds it by `NoteId` to the feature note it pays for, and the script requires that feature note to be consumed in the same transaction while leaving the assets in place for the consuming account to collect, so consumption rights are inherited from the feature note and the note demands no wallet interface from the account it sponsors. Any other consumption is a reclaim against the `reclaimer` stored in note storage (builder-settable, defaults to `sender`), gated on a `reclaim_height`; only the reclaim path moves assets ([#3274](https://github.com/0xMiden/protocol/pull/3274)).
