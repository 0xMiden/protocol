...
- [BREAKING] `TokenPolicyManager` now dispatches mint and burn policies via `dyncall` rather than `dynexec`, so they follow the `call` ABI and run in their own memory context ([#3014](https://github.com/0xMiden/protocol/issues/3014)).
