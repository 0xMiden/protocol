...
- [BREAKING] Mint policies can no longer modify the output note's `tag`, `note_type`, or `RECIPIENT`: they are passed for inspection only and the policy manager reverts the mint if a policy returns them changed ([#XXXX](https://github.com/0xMiden/protocol/issues/XXXX)).
