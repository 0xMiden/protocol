...
- [BREAKING] Moved the `fee_asset_id` slot from `ConstantFeePolicy` to `FeeManager` (now set via the required `FeeManagerBuilder::fee_faucet_id`), added a `get_fee_asset_id` account procedure, and changed fee policies' `compute_note_fee` to return only the fee value (`AssetValue`), with the manager prepending the fee asset ID ([#3347](https://github.com/0xMiden/protocol/pull/3347)).
