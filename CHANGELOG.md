...
- The PSWAP note script now bounds its lineage depth to a u32 and the `PswapNote` builder rejects a malformed `PswapAttachment`, so a hand-crafted starting depth can neither wrap the on-chain increment nor desynchronize the off-chain lineage helpers ([#3682](https://github.com/0xMiden/protocol/issues/3682)).
