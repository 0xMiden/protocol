...
### Changes
- Added the `NetworkSponsorshipNote` standard note and `FeeSponsorship` note attachment, and a `miden::standards::note::note_id` MASM module for computing a note's ID on-chain, letting a depth=1 sponsorship note bind to and release its assets only alongside the specific feature note it names by `NoteId`. Any other consumption is a reclaim against the `reclaimer` stored in note storage (builder-settable, defaults to `sender`), gated on a `reclaim_height` ([#3274](https://github.com/0xMiden/protocol/pull/3274)).
