...
- Fixed `RoleBasedAccessControl` role administration becoming permanently unmanageable when a role's admin was delegated to a memberless role, whether by delegating to a role that did not exist yet, to a mistyped symbol, or to a role that later lost its last member. A memberless role can authorize nothing, so authority over the roles it administers now falls back to the built-in `ADMIN` role: delegation is exclusive for as long as the delegated admin role keeps at least one member, and only emptying `ADMIN` itself is final ([#TBD](https://github.com/0xMiden/protocol)).
