...
- [BREAKING] `AccountBuilder` now rejects accounts that enable asset callbacks with a private account type, since holders of such an account's assets could not load its state to dispatch the callbacks ([#3717](https://github.com/0xMiden/protocol/issues/3717)).
