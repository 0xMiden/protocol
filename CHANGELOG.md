...
- [BREAKING] Split `create_fungible_faucet` into per-auth user-faucet factories — `create_singlesig_user_fungible_faucet(auth_component: AuthSingleSigAcl, ...)`, `create_multisig_user_fungible_faucet(auth_component: AuthMultisig, ...)`, and `create_guarded_user_fungible_faucet(auth_component: AuthGuardedMultisig, ...)` (all install `Authority::AuthControlled` directly) — and the opinionated `create_network_fungible_faucet(access_control, ...)` (always `AccountType::Public`, builds the `AuthNetworkAccount` allowlist internally from `MintNote` + `BurnNote` script roots and allowlists the canonical `ExpirationTransactionScript` in the tx-script allowlist). Other auth schemes / shapes are no longer supported through these helpers — fall back to `AccountBuilder` directly. `user_faucet_single_sig_acl`, `user_faucet_multisig`, and `user_faucet_guarded` testing helpers are provided behind the `testing` feature ([#2944](https://github.com/0xMiden/protocol/pull/2944)).
...
- [BREAKING] `FungibleFaucet` no longer installs the `is_paused` storage slot itself. Faucet factories (the `create_*_user_fungible_faucet` family / `create_network_fungible_faucet`) now bundle the `Pausable` component (slot + `is_paused()` view procedure) alongside `PausableManager`. Callers using `AccountBuilder` directly must also install `Pausable` or the faucet's mint / burn / transfer / metadata-setter procedures will panic at runtime ([#2944](https://github.com/0xMiden/protocol/pull/2944)).
