...
- Optimized `miden::standards::note::note_id::find_input_note_by_id` with the new `INPUT_NOTE_INDEX_REQUEST_EVENT`: executor and prover hosts now provide a candidate input-note index, successful lookups are authenticated against the `NoteId` cached by the transaction prologue, and reported misses are validated by a full scan. This makes the common found case O(1) in the VM while preserving soundness, and provides a shared fast lookup for `FeeSponsorshipNote` and the fee-collection work tracked in [#3340](https://github.com/0xMiden/protocol/issues/3340) ([#3424](https://github.com/0xMiden/protocol/pull/3424)).
