...
- [BREAKING] Added the FEE standard note (`FeeNote` / `miden::standards::notes::fee`): a public, untargeted P2ID-like note consumable by any account, intended for paying transaction fees to a batch builder. Includes MASM `fee::prepare_note` / `fee::create_output_note` creation procedures, the unique `0xFEE` note tag, and a `StandardNote::FEE` variant (breaking for exhaustive matches on `StandardNote`) ([#3117](https://github.com/0xMiden/protocol/issues/3117)).
