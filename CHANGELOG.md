...
### Changes
- Added an emergency switch via `Sentry` component into the `Authority`: with `Sentry` installed, `freeze` is authorized for the emergency authority or the sentry, while `unfreeze` (re-opening) remains restricted to the emergency authority. Exposed boolean `rbac::is_sender_in_role` / `rbac::is_sender_admin` primitives to support this ([#3253](https://github.com/0xMiden/protocol/pull/3253)).
...
- Added a standalone `Sentry` account component storing a single sentry account ID, with `get_sentry` / `set_sentry` procedures and `is_sender_sentry` / `assert_sender_is_sentry` authorization primitives ([#3125](https://github.com/0xMiden/protocol/pull/3125), [#3253](https://github.com/0xMiden/protocol/pull/3253)).
