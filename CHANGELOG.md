...
### Changes
- [BREAKING] Changed `AccountCode::from_parts` to return a `Result` instead of panicking on an invalid procedure count, and added an enforcement for procedure-root uniqueness when an `AccountCode` is constructed via `from_parts` or deserialized, rejecting codes whose procedure list repeats a root ([#3246](https://github.com/0xMiden/protocol/pull/3246)).
- [BREAKING] Changed `AuthMultisigConfig::proc_thresholds` to return a `BTreeMap` keyed by procedure root instead of a slice of pairs, with `with_proc_thresholds` rejecting duplicate procedure roots, and `AuthMultisig::new` rejecting per-procedure threshold overrides that exceed the threshold guarding `set_procedure_threshold` (such an override could be stripped by a smaller quorum) ([#3246](https://github.com/0xMiden/protocol/pull/3246)).
- Hardened the multisig components: `update_signers_and_threshold` (in both `multisig` and `multisig_smart`) now rejects signer sets containing duplicate approver public keys, `get_signer_at` reverts on an out-of-range index instead of returning an empty signer, and `multisig_smart::set_procedure_policy` rejects a procedure root that is not one of the account's procedures ([#3246](https://github.com/0xMiden/protocol/pull/3246)).
...
- Renamed the `account_id::is_equal` procedure to `account_id::eq` for consistency with the `account_id::eqz`/`testz` zero-test helpers ([#3271](https://github.com/0xMiden/protocol/pull/3271)).
