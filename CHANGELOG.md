...
- [BREAKING] Added AggLayer bridge and faucet account builders with `with_fee_policy_manager`, plus `NetworkNotePricer::agglayer_bridge_fee_policy_manager` / `agglayer_faucet_fee_policy_manager` constructors that create production `BasicConstantFeePolicy` schedules from benchmarked prices for every note those accounts consume; the existing account constructors now require the deployment-provided manager ([#3487](https://github.com/0xMiden/protocol/issues/3487)).
