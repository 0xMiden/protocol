...
- [BREAKING] Bound the non-fungible MINT note to its faucet. A non-fungible `ASSET_ID` is derived from the consuming account at mint time, so unlike the fungible path's stored `ASSET_ID` it could not bind the note to one faucet, and any account exposing a non-fungible `mint_and_send` could consume a MINT note addressed to a different faucet -- spending the nullifier and permanently denying the note to its faucet. The non-fungible storage variants now carry the faucet ID, which the script asserts against the consuming account before minting, mirroring the fungible `ASSET_ID` comparison. `MintNote::NON_FUNGIBLE_NUM_STORAGE_ITEMS_PRIVATE` grows from 9 to 11; the public variant stays at 16+ because the faucet ID occupies two of the felts that padded the tag out to a word boundary ([#3474](https://github.com/0xMiden/protocol/pull/3474)).
