...
- [BREAKING] The `AuthGuardedMultisig` component now pays the transaction fee, and `tx_policy::assert_no_output_notes` / `guardian::verify_signature` now take the output note count sampled at the start of the authentication procedure so the fee note does not trip the guardian rotation guard. The component's code commitment changes ([#XXXX](https://github.com/0xMiden/protocol/pull/XXXX)).
