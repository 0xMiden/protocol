...
- [BREAKING] Updated the Miden VM and crypto crate family to v0.30.0 and `midenc-hir-type` to v0.12.0. `LocalTransactionProver::new` now takes `miden_prover::Prover`, `CoreLibrary` exposes one merged package, and transaction and batch verification reject proofs with outstanding precompile obligations ([#TBD](https://github.com/0xMiden/protocol/pull/TBD)).
- [BREAKING] Updated the Miden VM and crypto crate family to v0.30.0 and `midenc-hir-type` to v0.12.0. `LocalTransactionProver::new` now takes `miden_prover::Prover`, `CoreLibrary` exposes one merged package, and transaction and batch verification reject proofs with outstanding precompile obligations.
- [BREAKING] Updated the Miden VM and crypto crate family to v0.30.0 and `midenc-hir-type` to v0.12.0. `LocalTransactionProver::new` now takes `miden_prover::Prover`, `CoreLibrary` exposes one merged package, and transaction and batch verification reject proofs with outstanding precompile obligations ([#3782](https://github.com/0xMiden/protocol/pull/3782)).
