...
- [BREAKING] Flipped `AuthSingleSigAcl` ACL semantics from a trigger list to an exempt list: every called account procedure now requires a signature unless its root is listed in the new `exempt_procedures` configuration. Removed the `allow_unauthorized_output_notes` and `allow_unauthorized_input_notes` flags (the note checks are now implicit) and the `config` storage slot. The trigger-roots storage map was renamed to `exempt_procedure_roots`. Part of [#2964](https://github.com/0xMiden/protocol/issues/2964).
