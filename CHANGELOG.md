...
- [BREAKING] Extended `Authority::RbacControlled` to assign roles per authority-gated procedure (keyed by procedure root, resolved at runtime via `caller`), with a `DefaultAuthority` fallback (owner or default role) for unmapped procedures. Enables, e.g., `pause` → `PAUSER` and `unpause` → `UNPAUSER` on a single account ([#2925](https://github.com/0xMiden/protocol/pull/2925)).
