...
- Narrowed the `AuthRequest` gate introduced in [#3233](https://github.com/0xMiden/protocol/pull/3233) to signature *production* only: verifying an externally-supplied signature (which never uses the account's private key) is now permitted outside the authentication procedure, while production stays restricted to it. This keeps the fix intact and unblocks account procedures that verify co-signer signatures before the epilogue ([#TBD](https://github.com/0xMiden/protocol/pull/TBD)).
