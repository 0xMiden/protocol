...
- [BREAKING] Renamed `ConstantFeePolicy` to `BasicConstantFeePolicy`, indicating it is the simplest constant fee policy rather than an authoritative one; the MASM module is now `miden::standards::fees::policies::basic_constant_fee`, the component namespace is `miden::standards::components::fees::policies::basic_constant_fee`, and the `fee_schedule` storage slot name follows suit ([#3391](https://github.com/0xMiden/protocol/issues/3391)).
