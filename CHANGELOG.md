...
### Changes
- Added the `FeeManager` and `FeeAuth` account components, the `FeeNote` and `NetworkSponsorshipNote` note types, and the `FeeSponsorship` note attachment, implementing network-account fee sponsorship (approach C): a feature note stays fee-unaware while its fee travels in a separate sponsorship note bound to it by note ID, settled in the account's auth procedure ([#3270](https://github.com/0xMiden/protocol/pull/3270)).
