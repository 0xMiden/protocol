...
- [BREAKING] Unified the two account-origin authenticators in the transaction kernel's `api.masm` into a single `authenticate_account_origin` procedure that conditionally tracks the call: a kernel procedure call is recorded (via the `was_called` flag) only when the active account is the native account and the account's authentication procedure is not currently executing. As a result the read-only introspection procedures (`account_has_procedure`, `account_get_procedure_root`, `account_get_num_procedures`, `account_get_code_commitment`) and the indexed (non-active-note) `input_note_remove_asset` / `input_note_remove_all_assets` path are now tracked in the native context, which changes their `was_procedure_called` results, and `account_upgrade` is now gated by the authenticator (it panics when not invoked from the account context) ([#3310](https://github.com/0xMiden/protocol/issues/3310)).
