...
- [BREAKING] `tx_policy::assert_no_output_notes` now reads the transaction's output note count from the kernel and takes the number of notes the caller created itself, asserting the two are equal; it previously asserted on a caller-supplied count it never verified, so a caller passing zero disabled the check. `auth::guardian::verify_signature` and `multisig_smart::{auth_tx, enforce_note_restrictions}` take that count through, so a component's own fee notes no longer trip the guardian rotation guard or a `NoOutputNotes` procedure policy ([#3757](https://github.com/0xMiden/protocol/issues/3757)).
- [BREAKING] Split the payment out of `fee::pay_fee` into `fee::resolve_payment_info` and `fee::pay_estimated_fee`, so a caller can insert `fee::assert_fee_bound` between the resolved payment and the vault withdrawal that honours it; `pay_fee` is now the composition of the estimate, the sponsorship payment and the two, and its inputs, outputs and behaviour are unchanged, but its MAST root and therefore the code commitment of every fee-paying component change ([#3332](https://github.com/0xMiden/protocol/issues/3332)).
