...
- [BREAKING] Moved every account component's metadata (name, description, version and storage schema) from Rust into a `[package.metadata.account-component]` table in its `miden-project.toml`, embedded it into the component's `.masp` package so it travels with the compiled component, and removed the now-redundant `*_slot_schema` accessors ([#TBD](https://github.com/0xMiden/protocol/issues/TBD)).
