...
- [BREAKING] `FeeSponsorshipNote` now wraps the underlying `Note`, is parsed back from one with `TryFrom<Note>`, carries a fungible fee asset, and replaces its `target_id` accessor with `tag` ([#3746](https://github.com/0xMiden/protocol/pull/3746)).
