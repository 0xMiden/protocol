...
- Documented that values read through FPI reflect the executor chosen reference block and are not revalidated against current foreign account state at inclusion, so they may be outdated, and recommended that authors of foreign account procedures set a transaction expiration delta when recent reads are required ([#3208](https://github.com/0xMiden/protocol/pull/3208)).
