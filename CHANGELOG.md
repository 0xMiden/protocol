...
- Extended the standardized `NetworkAccountConfig` note with `AddAllowedFeePolicy` / `RemoveAllowedFeePolicy` actions, letting a network account manage its allowed fee policy roots post-deployment via the authority-gated `add_allowed_fee_policy` / `remove_allowed_fee_policy` procedures ([#3325](https://github.com/0xMiden/protocol/issues/3325)). The note is also bound to its target account by a `NetworkAccountTarget` attachment, verified on-chain by the note script, so a decoy account cannot consume (and burn) a note meant for another account ([#3433](https://github.com/0xMiden/protocol/issues/3433)).
