...
- [BREAKING] Removed the fee asset ID storage from `ConstantFeePolicy`; the policy now reads the fee asset ID from the `FeeManager` installed on the same account, so `ConstantFeePolicy::new` no longer takes a fee faucet ID ([#3328](https://github.com/0xMiden/protocol/issues/3328)).
