...
- Renamed the `account_id::is_equal` procedure to `account_id::eq` for consistency with the `account_id::eqz`/`testz` zero-test helpers, and replaced the bridge's remaining verbose zero-check with `account_id::testz` ([#3271](https://github.com/0xMiden/protocol/pull/3271)).
