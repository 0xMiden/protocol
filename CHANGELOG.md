...
- The batch kernel now verifies the batch's transaction list against its `BATCH_ID`, verifies each transaction header, and computes the batch's `INPUT_NOTES_COMMITMENT` as the nullifier-sorted commitment matching `ProposedBatch::input_notes().commitment()`, which `BatchVerifier` now checks ([#2905](https://github.com/0xMiden/protocol/pull/2905)).
