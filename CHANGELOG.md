...
### Changes
- [BREAKING] Replaced the owner-only transfer allowlist/blocklist admin components (`AllowlistOwnerControlled` / `BlocklistOwnerControlled`) with authority-gated `AllowlistManager` / `BlocklistManager`, mirroring `PausableManager`. Their admin procedures (`allow_account` / `disallow_account`, `block_account` / `unblock_account`) now gate on the account-wide `Authority` component via `assert_authorized`, so they work under `AuthControlled`, `OwnerControlled`, and `RbacControlled`. Under RBAC, mapping both of a list's procedure roots to one role symbol (e.g. `ALLOWLISTER` / `BLOCKLISTER`) lets a single role gate both operations; the new `AllowlistManager::allow_account_root` / `disallow_account_root` and `BlocklistManager::block_account_root` / `unblock_account_root` accessors key the role map ([#3014](https://github.com/0xMiden/protocol/issues/3014)).
