...
- [BREAKING] `multisig_smart` now rejects delay-only procedure policies, which policy enforcement (always immediate) would have turned into a permanent brick of the policied procedure: `set_procedure_policy` requires a non-zero immediate threshold alongside any delayed threshold, and `ProcedurePolicyExecutionMode::DelayOnly` and `ProcedurePolicy::with_delay_threshold` were removed ([#PR](https://github.com/0xMiden/protocol/pull/PR)).
