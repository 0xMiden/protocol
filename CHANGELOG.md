...
### Changes
- Added the `FeeManager` and `AuthNetworkAccountWithFees` account components and the `FeeNote` and `NetworkSponsorshipNote` note types, implementing network-account fee sponsorship (approach C): a feature note stays fee-unaware while its fee travels in a separate, attachment-free sponsorship note bound to it by the `NoteId` in its note storage, settled in the account's auth procedure. Chained network notes pass the remaining budget downstream, including self-spawning chains that run until the budget is exhausted ([#3270](https://github.com/0xMiden/protocol/pull/3270)).
- Added the `NetworkSponsorshipNote` standard note and a `miden::standards::note::note_id` MASM module for computing a note's ID on-chain. The sponsorship note carries no attachments: its note storage binds it by `NoteId` to the feature note it pays for, and the script releases its assets only in a transaction that also consumes that feature note, so consumption rights are inherited from the feature note. Any other consumption is a reclaim against the `reclaimer` stored in note storage (builder-settable, defaults to `sender`), gated on a `reclaim_height` ([#3274](https://github.com/0xMiden/protocol/pull/3274)).
