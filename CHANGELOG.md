...
- [BREAKING] Added an emergency pause to the AggLayer bridge via the standards `Pausable`/`PausableManager` components: all bridge entry points except `remove_ger` abort while paused, and the `ADMIN`-gated standards `PAUSE_ACTION` note toggles the state (not yet consume-side bound to the bridge, see [#3433](https://github.com/0xMiden/protocol/issues/3433)); the bridge code commitment and note allowlist change ([#2696](https://github.com/0xMiden/protocol/issues/2696)).
