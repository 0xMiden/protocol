...
- Added `(order_id, depth)` fields to PSWAP output note attachments under a new `PswapAttachment` scheme (value 3), enabling PSWAP creators to disambiguate same-creator outputs in a single transaction and reconstruct private payback / remainder notes from on-chain headers alone. Adds `PswapNote::order_id` and `PswapNote::parent_depth` accessors, plus `PswapNote::payback_note(consumer_account_id, depth, fill_amount)` and `PswapNote::remainder_note(consumer_account_id, depth, payout_amount, remaining_offered, remaining_requested)` discovery helpers that reconstruct the depth-`d` output notes byte-for-byte — matching both `Note::id` and `Note::commitment` — so the creator can consume them as unauthenticated input notes against a chain holding only the on-chain commitments. The required `consumer_account_id` and `payout_amount` arguments are themselves recoverable from the on-chain note (`metadata().sender()` and the first felt of the attachment word), so no out-of-band identity is needed ([#2909](https://github.com/0xMiden/protocol/pull/2909)).
- Added `PswapAttachment` scheme on PSWAP output notes carrying `[amount, order_id, depth, 0]`, plus `PswapNoteAttachment`, `PswapNote::order_id` / `parent_depth` accessors and `payback_note` / `remainder_note` discovery helpers that reconstruct on-chain output notes byte-identically, so creators can consume private paybacks from on-chain commitments alone ([#2909](https://github.com/0xMiden/protocol/pull/2909)).
