...
- [BREAKING] The `AuthGuardedMultisig` component now pays the transaction fee, which changes its code commitment and requires the transaction's auth args to commit to the fee conversion info ([#3757](https://github.com/0xMiden/protocol/issues/3757), [#3758](https://github.com/0xMiden/protocol/pull/3758)).
- [BREAKING] The standards procedures `tx_policy::assert_no_output_notes` and `auth::guardian::verify_signature` now take the output note count sampled at the start of the authentication procedure, so a component's own fee note no longer trips the guardian rotation guard; since components link the standards library statically, this also changes the code commitment of `AuthMultisigSmart` ([#3757](https://github.com/0xMiden/protocol/issues/3757), [#3758](https://github.com/0xMiden/protocol/pull/3758)).
- [BREAKING] `fee::pay_fee` now requires the fee to be paid in the native fee asset and bounds the paid amount to at most twice the computed fee, so a caller-supplied conversion rate can no longer drain the vault through the fee note — closing a fee-based account-draining vector via reduced-quorum multisig and guardian-rotation authorization; paying fees in a non-native asset is rejected until a fee rate oracle exists ([#3765](https://github.com/0xMiden/protocol/pull/3765), [#3763](https://github.com/0xMiden/protocol/issues/3763)).
