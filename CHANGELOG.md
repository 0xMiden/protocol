...
### Changes
- Added a global emergency switch to `Authority`: owner-gated `set_target_closed` / `set_target_opened` procedures toggle a `target_closed` flag that makes `assert_authorized` block every authority-gated procedure at once, regardless of role or owner membership ([#XXXX](https://github.com/0xMiden/protocol/pull/XXXX)).
