...
- [BREAKING] `NetworkAccountTarget` decoding no longer discards the target account ID when the execution hint slot holds an unrecognized encoding, which hid such notes from network routing even though the on-chain targeting path ignores that slot; `execution_hint` now returns `Option<NoteExecutionHint>` ([#TODO](https://github.com/0xMiden/protocol/pull/TODO)).
