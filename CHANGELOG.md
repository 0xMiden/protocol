...
- [BREAKING] Added an emergency pause to the AggLayer bridge via the standards `Pausable`/`PausableManager` components: all bridge entry points except `remove_ger` abort while paused, and the new `ADMIN`-gated, target-bound `PAUSE_AGG_BRIDGE` note (`PauseAggBridgeNote`) toggles the state; the bridge code commitment and note allowlist change ([#2696](https://github.com/0xMiden/protocol/issues/2696)).
