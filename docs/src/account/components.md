...
Simple schemas accept `word` (default) and word-shaped types such as `miden::standards::auth::signature::pub_key` (parsed from hexadecimal strings).
...
In our TOML example, the first entry defines a composite schema, while the second is an init-supplied value typed as `miden::standards::auth::signature::pub_key`.
...
[[storage.slots]]
name = "demo::typed_map"
type = { key = "word", value = "miden::standards::auth::signature::pub_key" }
```

This declares that all keys are `word` and all values are `miden::standards::auth::signature::pub_key`, regardless of whether the map contents come from `default-values = [...]` (static) or are supplied at instantiation via `InitStorageData`.
