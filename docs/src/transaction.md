...
4. **Epilogue**
   ...
   - The account's [authentication procedure](account/code#authentication) is called to authorize the transaction. As part of authorization, the procedure pays the transaction fee by creating a public TX_FEE note funded from the account's vault, in the chain's native fee asset and for the amount the kernel computed. The payment asset and its conversion rate to the native fee asset are still user-supplied via the auth args, so a signature covers them, but the standard `pay_fee` procedure asserts that they name the reference block's fee asset at rate 1/1. See [Fees](fees.md).
