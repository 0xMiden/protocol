...
- Values read from a foreign account through foreign procedure invocation reflect that account's state at the transaction's reference block, which the executor (prover) chooses and which may be an older canonical block. Unlike the native account's initial state, a foreign read is bound only to the reference block: the foreign account commitment is not part of the transaction's public inputs and is never revalidated against the foreign account's current on-chain state when the `Transaction` is included in a block. Integrators must not rely on foreign reads for decisions that require the foreign account's present value (for example cross-account authorization such as roles or allowlists, or oracle and price checks), since the party proving the `Transaction` can anchor it to a past block in which the value was outdated but favorable. In practice an executor usually has an interest in picking a recent enough reference block to make the decision correctly, but this is not enforced by the protocol. Authors of foreign account procedures that must not be read when outdated should set a transaction expiration delta, which bounds how old the reference block can be relative to the block the `Transaction` is included in. For example, if the current block is 40 and the foreign account sets a delta of 5, the reference block must be block 35 or newer, so a value from block 20 could not be read.
