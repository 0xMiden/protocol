...
| `remove_asset`      | Removes an [asset](note.md#assets) from the input note with the specified index and returns the value remaining in the note, which is the empty word if the entire asset was removed.<br/><br/>**Inputs:** `[ASSET_ID, ASSET_VALUE, note_index]`<br/>**Outputs:** `[FINAL_ASSET_VALUE]` | Any     |
| `remove_all_assets` | Removes all remaining [assets](note.md#assets) from the input note with the specified index and writes them into memory starting at the specified address.<br/><br/>**Inputs:** `[dest_ptr, note_index]`<br/>**Outputs:** `[num_assets]` | Any     |
...
| `compute_and_store_recipient_from_commitment` | Computes the recipient from the serial number, script root, and storage commitment, and stores the recipient preimages in advice inputs.<br/><br/>**Inputs:** `[SERIAL_NUM, SCRIPT_ROOT, STORAGE_COMMITMENT]`<br/>**Outputs:** `[RECIPIENT]` | Any     |
