...
| `remove_asset`      | Removes an [asset](note.md#assets) from the input note with the specified index and returns the value remaining in the note, which is the empty word if the entire asset was removed.<br/><br/>**Inputs:** `[ASSET_ID, ASSET_VALUE, note_index]`<br/>**Outputs:** `[FINAL_ASSET_VALUE]` | Account |
| `remove_all_assets` | Removes all remaining [assets](note.md#assets) from the input note with the specified index and writes them into memory starting at the specified address.<br/><br/>**Inputs:** `[dest_ptr, note_index]`<br/>**Outputs:** `[num_assets]` | Account |
