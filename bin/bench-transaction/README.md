...
- standards, consumed by a network account with the matching management components: FAUCET_POLICY_ACTION, ALLOWLIST_CONFIG, BLOCKLIST_CONFIG, PAUSE_ACTION, OWNER_ACTION, RBAC_ACTION, NETWORK_ACCOUNT_CONFIG (one representative action selector each; other selectors run the identical dispatch path)
