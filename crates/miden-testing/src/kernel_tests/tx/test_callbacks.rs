...
/// Tests that the account callback cannot rewrite the value added to the account vault.
///
/// The callback attempts to replace 200 units with 100 by leaving that value in its return frame.
/// The kernel must ignore the frame and add the original 200 units to the vault.
#[tokio::test]
async fn test_callback_cannot_rewrite_value_added_to_account() -> anyhow::Result<()> {
    ...
    @account_procedure
    pub proc on_before_asset_added_to_account
        # Attempt to return 100 in place of the original 200 units.
        dropw
        neg add.300
        # => [WOULD_BE_REPLACEMENT_VALUE, pad(8)]
    end
    "#;

    let faucet = add_faucet_with_callbacks(&mut builder, Some(account_callback_masm), None)?;
    let original_asset = FungibleAsset::new(faucet.id(), 200)?;
    let note = builder.add_p2id_note(
        faucet.id(),
        target_account.id(),
        &[original_asset.into()],
    ...
    let updated_assets: Vec<_> = executed.account_patch().vault().updated_assets().collect();
    assert_eq!(updated_assets, vec![Asset::Fungible(original_asset)]);
