...
/// One feature note whose fee is not covered makes [`NoteConsumptionChecker`] report the intact
/// (feature note, FEE_SPONSORSHIP) pairs sharing its batch as failed too.
///
/// The uncovered note aborts fee collection in the auth procedure, which runs after note
/// processing and so is reported as an epilogue failure. That routes the batch into a search that
/// grows its candidate set one note at a time, and neither half of a pair is consumable alone, so
...
async fn note_checker_drops_intact_pairs_alongside_an_uncovered_note(
    ...
    assert!(
        successful.is_empty(),
        "the intact pair is dropped along with the uncovered note"
    );
