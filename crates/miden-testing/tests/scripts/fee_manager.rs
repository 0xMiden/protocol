...
// NETWORK ACCOUNT CONFIG NOTE — FEE POLICY ACTIONS
// ================================================================================================

/// Builds a standardized `NetworkAccountConfigNote` triggering `action` on `account`, sent by
...
/// The standardized `NetworkAccountConfigNote` drives `add_allowed_fee_policy`: after the owner
...
/// The standardized `NetworkAccountConfigNote` drives `remove_allowed_fee_policy`: after the
...
async fn config_note_removes_allowed_fee_policy_root() -> anyhow::Result<()> {
    ...
    let remove_note = build_config_note(
        owner_account_id,
        account.id(),
        NetworkAccountConfig::RemoveAllowedFeePolicy { policy_root: allowed_root },
        810,
    )?;
    let set_note = build_sender_note(
        owner_account_id,
