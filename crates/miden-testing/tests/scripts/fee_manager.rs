...
// CREATE SPONSORSHIP NOTES
// ================================================================================================

const NETWORK_SENDER_NAME: &str = "test::network_sender";

/// The source of a test-only account procedure that creates one P2ID network output note (targeted
/// at `network_account_id` and marked with a `NetworkAccountTarget` attachment) and then runs
/// `create_sponsorship_notes`, mirroring how a sending account would auto-sponsor its network
/// notes. The target account ID is baked in because `create_sponsorship_notes` prices the note
/// through that account's fee policy via FPI.
fn network_sender_source(network_account_id: AccountId) -> String {
    format!(
        r#"
        ...
        @account_procedure
        pub proc create_network_note_and_sponsor
            # build a P2ID network output note targeted at the network account (arbitrary serial)
            push.7.7.7.7
            push.{note_type}
            push.0
            push.{net_prefix} push.{net_suffix}
            ...
            # mark it as a network note targeting the network account
            push.0 push.{net_prefix} push.{net_suffix}
            # => [net_suffix, net_prefix, exec_hint_tag, network_note_idx]

            exec.network_account_target::new
            # => [attachment_scheme, NOTE_ATTACHMENT, network_note_idx]

            exec.output_note::add_word_attachment
            # => []

            exec.fee_manager::create_sponsorship_notes
            # => []
        end
        "#,
        note_type = NoteType::Public as u8,
        net_prefix = network_account_id.prefix().as_felt(),
        net_suffix = network_account_id.suffix(),
    )
}

/// `create_sponsorship_notes` pairs each network output note with a FEE_SPONSORSHIP note carrying
/// the fee the target account's policy charges, funded from the sender's vault.
#[tokio::test]
async fn create_sponsorship_notes_pairs_network_note_with_sponsorship() -> anyhow::Result<()> {
    // The target network account prices the P2ID script root, so the network note the sender emits
    // is priced at FEE_AMOUNT.
    let network = network_account(Some(P2idNote::script_root()))?;
    let fee = fee_asset(FEE_AMOUNT)?;

    // Compile the sender component once; it is both installed on the account and dynamically linked
    // into the transaction script that calls into it.
    let sender_code = CodeBuilder::default()
        .compile_component_code(NETWORK_SENDER_NAME, network_sender_source(network.id()))?;
    let sender_component = AccountComponent::new(
        sender_code.clone(),
        vec![],
        AccountComponentMetadata::mock(NETWORK_SENDER_NAME),
    )?;

    let mut builder = MockChain::builder();
    builder.add_account(network.clone())?;

    let sender_builder = Account::builder([9u8; 32])
        .account_type(AccountType::Public)
        .with_component(BasicWallet)
        .with_component(sender_component)
        .with_assets([fee]);
    let sender =
        builder.add_account_from_builder(Auth::IncrNonce, sender_builder, AccountState::Exists)?;

    let mut mock_chain = builder.build()?;
    mock_chain.prove_next_block()?;

    let tx_script = CodeBuilder::default()
        .with_dynamically_linked_library(&sender_code)?
        .compile_tx_script(
            r#"
            use test::network_sender

            @transaction_script
            pub proc main
                call.network_sender::create_network_note_and_sponsor
                # => [pad(16)]
    ...
    let foreign_inputs = mock_chain.get_foreign_account_inputs(network.id())?;

    let executed = mock_chain
        .build_tx_context(sender.id(), &[], &[])?
    ...
    assert_eq!(
        ...
        2,
        "the network note and its sponsorship should be output"
    );

    let mut sponsorship = None;
    let mut network_note = None;
    for raw in output_notes.iter() {
        let RawOutputNote::Full(note) = raw else {
            anyhow::bail!("expected full output notes");
        };
        if note.script().root() == FeeSponsorshipNote::script_root() {
            sponsorship = Some(note.clone());
        } else {
            network_note = Some(note.clone());
        }
    }

    let sponsorship = sponsorship.expect("a FEE_SPONSORSHIP note should be created");
    let network_note = network_note.expect("the P2ID network note should be created");

    // the sponsorship carries exactly the fee the policy priced the network note at
    assert_eq!(sponsorship.assets().num_assets(), 1);
    assert_eq!(sponsorship.assets().iter().next(), Some(&fee));

    // and it is bound to the network note as its companion
    let storage = FeeSponsorshipNoteStorage::try_from(sponsorship.storage().items())?;
    assert_eq!(storage.feature_note_id(), network_note.id());
