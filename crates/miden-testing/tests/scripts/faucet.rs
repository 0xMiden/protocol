...
async fn network_faucet_mint() -> anyhow::Result<()> {
    ...
    let mint_storage =
        MintNoteStorage::new_fungible_private(recipient, mint_asset, output_note_tag.into());
...
async fn test_mint_note_output_note_types(#[case] note_type: NoteType) -> anyhow::Result<()> {
    ...
    let mint_storage = match note_type {
        ...
        NoteType::Public => {
            ...
            MintNoteStorage::new_fungible_public(recipient, mint_asset, output_note_tag.into())?
        },
    };
