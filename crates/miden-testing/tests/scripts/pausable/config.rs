...
/// Decoy: same `PausableManager` setup and owner as the note's real target, so sender-based
/// authorization would pass; only the target check stops it, before the pause state changes.
pub(crate) fn decoy_scenario() -> anyhow::Result<crate::DecoyScenario> {
    let owner = AccountIdBuilder::new().build_with_seed([1; 32]);

    let (decoy_id, mock_chain) = crate::chain_with_decoy(create_pausable_account(owner)?)?;
    let mut rng = RandomCoin::new([Felt::from(100u32); 4].into());
    ...
    Ok(crate::DecoyScenario {
        ...
        note,
        target,
        expected_error: ERR_PAUSE_CONFIG_TARGET_ACCOUNT_MISMATCH,
    })
}
