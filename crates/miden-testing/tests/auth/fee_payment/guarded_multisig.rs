...
/// component does, and must do so within the cycle estimate it hands to `pay_fee`.
///
...
/// approvers. An estimate that is too low is the dangerous direction, since it under-prices the
/// fee.
///
/// Two assertions here point in opposite directions, and only one of them can see the extra
/// signer. The upper bound on measured cycles is a budget regression check: the measured count does
...
/// `guardian::verify_signature` calls `tx_policy::assert_no_output_notes`, which read the live
/// output-note counter. The account's own mandatory fee note therefore tripped a guard meant for
/// user notes, and a lost or compromised guardian key could never be rotated. The count is now
/// sampled at the start of the authentication procedure instead, which excludes the fee note
/// without excluding the transaction's own notes.
///
/// The two cases pin different things, and only the first pins the sampling. `without_output_note`
/// fails without this change, because the fee note alone trips the live counter. `with_output_note`
/// fails whether the count is sampled or live — one user note plus one fee note is non-zero either
/// way — so it is not evidence that sampling works; it guards the other direction, that sampling
/// did not weaken the check into accepting a user note. Note also that the fee note could not have
/// been caught by the neighbouring `assert_only_one_non_auth_procedure_called`, since it is created
/// outside any account procedure, which is why the count had to be threaded rather than the guard
/// left to catch it.
///
...
/// that loses its guardian key while holding no payment asset is therefore unrecoverable, which is
/// a property of paying the fee from the authentication procedure rather than an oversight in the
/// rotation guard.
#[tokio::test]
