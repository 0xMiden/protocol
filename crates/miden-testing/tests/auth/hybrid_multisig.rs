...
use miden_standards::account::auth::{AuthMultisig, AuthMultisigConfig};
...
#[tokio::test]
async fn test_multisig_set_procedure_threshold() -> anyhow::Result<()> {
    let (_secret_keys, auth_schemes, public_keys, authenticators) =
        setup_keys_and_authenticators(2, 2)?;
...
#[tokio::test]
async fn test_multisig_set_procedure_threshold_rejects_exceeding_approvers() -> anyhow::Result<()> {
    let (_secret_keys, auth_schemes, public_keys, authenticators) =
        setup_keys_and_authenticators(2, 2)?;
...
#[tokio::test]
async fn test_hybrid_multisig_update_psm_public_key() -> anyhow::Result<()> {
    let (_secret_keys, auth_schemes, public_keys, authenticators) =
        setup_keys_and_authenticators(2, 2)?;
    let approvers = public_keys
        .iter()
...
#[tokio::test]
async fn test_hybrid_multisig_update_psm_public_key_requires_capability() -> anyhow::Result<()> {
    let (_secret_keys, auth_schemes, public_keys, _authenticators) =
        setup_keys_and_authenticators(2, 2)?;
    let approvers = public_keys
        .iter()
