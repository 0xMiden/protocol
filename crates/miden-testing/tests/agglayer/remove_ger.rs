...
use super::test_utils::{BridgeSetup, keccak256_felts, setup_bridge};
...
fn fold_removed_ger_chain(prev_chain: [u8; 32], ger_bytes: [u8; 32]) -> [u8; 32] {
    let mut preimage = [0u8; 64];
    preimage[..32].copy_from_slice(&prev_chain);
    preimage[32..].copy_from_slice(&ger_bytes);
    let chain_felts = keccak256_felts(&preimage);
    ...
    for (i, felt) in chain_felts.iter().enumerate() {
        ...
        chain_bytes[i * 4..(i + 1) * 4].copy_from_slice(&limb.to_le_bytes());
    }
    chain_bytes
}
...
async fn remove_ger_middle_of_multi_insert_leaves_others_intact() -> anyhow::Result<()> {
    ...
    assert!(
        !AggLayerBridge::is_ger_registered(ger_b, &updated_bridge_account)?,
        "GER B should have been removed"
    );
    assert!(
        AggLayerBridge::is_ger_registered(ger_c, &updated_bridge_account)?,
...
/// Tests two successful sequential removals: inserts GERs A and B, removes A, then removes B,
/// and verifies the chain folds over the non-zero intermediate value, i.e. ends at
/// `keccak256(keccak256(0...0 || A) || B)`.
#[tokio::test]
async fn remove_ger_sequential_removals_fold_chain() -> anyhow::Result<()> {
    ...
    builder.add_output_note(RawOutputNote::Full(update_a.clone()));
    builder.add_output_note(RawOutputNote::Full(update_b.clone()));
    builder.add_output_note(RawOutputNote::Full(remove_a.clone()));
    builder.add_output_note(RawOutputNote::Full(remove_b.clone()));
    ...
    assert!(
        !AggLayerBridge::is_ger_registered(ger_a, &updated_bridge_account)?,
        "GER A should have been removed"
    );
    assert!(
        !AggLayerBridge::is_ger_registered(ger_b, &updated_bridge_account)?,
        "GER B should have been removed"
...
async fn remove_ger_then_reinsert_succeeds() -> anyhow::Result<()> {
    ...
    assert!(
        AggLayerBridge::is_ger_registered(ger, &updated_bridge_account)?,
