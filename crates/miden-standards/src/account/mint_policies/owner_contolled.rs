...
use crate::account::components::owner_contolled_library;
...
procedure_digest!(
    ...
    OwnerContolled::NAME,
    OwnerContolled::OWNER_ONLY_PROC_NAME,
...
/// Initial policy configuration for the [`OwnerContolled`] component.
...
impl OwnerContolled {
    ...
    pub const NAME: &'static str = "miden::standards::components::mint_policies::owner_contolled";

    const OWNER_ONLY_PROC_NAME: &str = "owner_only";

    /// Creates a new [`OwnerContolled`] component from the provided configuration.
    ...
    /// Creates a new [`OwnerContolled`] component with owner-only policy as default.
...
impl Default for OwnerContolled {
    fn default() -> Self {
        Self::owner_only()
    }
}

impl From<OwnerContolled> for AccountComponent {
    fn from(owner_contolled: OwnerContolled) -> Self {
        let active_policy_proc_root_slot = StorageSlot::with_value(
            OwnerContolled::active_policy_proc_root_slot().clone(),
            owner_contolled.initial_policy_root,
        );
        ...
        if owner_contolled.initial_policy_root != owner_only_policy_root {
            allowed_policy_entries.push((
                StorageMapKey::from_raw(owner_contolled.initial_policy_root),
                allowed_policy_flag,
            ));
        }
        ...
        let metadata =
            AccountComponentMetadata::new(OwnerContolled::NAME, [AccountType::FungibleFaucet])
                .with_description(
