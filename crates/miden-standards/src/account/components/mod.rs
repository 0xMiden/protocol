...
static MINT_POLICY_MANAGER_LIBRARY: LazyLock<Library> = LazyLock::new(|| {
    let bytes = include_bytes!(concat!(
        env!("OUT_DIR"),
        "/assets/account_components/mint_policies/policy_manager.masl"
    ));
...
static OWNER_ONLY_MINT_POLICY_LIBRARY: LazyLock<Library> = LazyLock::new(|| {
    let bytes = include_bytes!(concat!(
        env!("OUT_DIR"),
        "/assets/account_components/mint_policies/owner_controlled/owner_only.masl"
    ));
    Library::read_from_bytes(bytes)
        .expect("Shipped `owner_only` Mint Policy library is well-formed")
});

// Initialize the Burn Policy Manager library only once.
static BURN_POLICY_MANAGER_LIBRARY: LazyLock<Library> = LazyLock::new(|| {
    let bytes = include_bytes!(concat!(
        env!("OUT_DIR"),
        "/assets/account_components/burn_policies/policy_manager.masl"
    ));
...
static OWNER_ONLY_BURN_POLICY_LIBRARY: LazyLock<Library> = LazyLock::new(|| {
    let bytes = include_bytes!(concat!(
        env!("OUT_DIR"),
        "/assets/account_components/burn_policies/owner_controlled/owner_only.masl"
    ));
    Library::read_from_bytes(bytes)
...
/// Returns the Burn Policy Manager Library.
pub fn burn_policy_manager_library() -> Library {
    BURN_POLICY_MANAGER_LIBRARY.clone()
}
