...
use crate::account::components::timed_unlimited_fungible_faucet_library;
...
// TIMED UNLIMITED FUNGIBLE FAUCET ACCOUNT COMPONENT
// ================================================================================================

procedure_digest!(
    TIMED_UNLIMITED_FUNGIBLE_FAUCET_DISTRIBUTE,
    TimedUnlimitedFungibleFaucet::DISTRIBUTE_PROC_NAME,
    timed_unlimited_fungible_faucet_library
);

procedure_digest!(
    TIMED_UNLIMITED_FUNGIBLE_FAUCET_BURN,
    TimedUnlimitedFungibleFaucet::BURN_PROC_NAME,
    timed_unlimited_fungible_faucet_library
);

/// An [`AccountComponent`] implementing a timed unlimited fungible faucet.
///
/// It reexports the procedures from `miden::standards::faucets::timed_unlimited_fungible`. When
...
///
/// No supply cap is enforced — minting is unrestricted up to protocol limits
/// ([`FungibleAsset::MAX_AMOUNT`](miden_protocol::asset::FungibleAsset::MAX_AMOUNT)),
/// but distribution is time-bounded by `distribution_end_block`.
///
...
pub struct TimedUnlimitedFungibleFaucet {
    metadata: TokenMetadata,
    distribution_end: u32,
    owner_account_id: AccountId,
}

impl TimedUnlimitedFungibleFaucet {
    // CONSTANTS
    // --------------------------------------------------------------------------------------------

    /// The name of the component.
    pub const NAME: &'static str = "miden::timed_unlimited_fungible_faucet";

    /// The maximum number of decimals supported by the component.
    pub const MAX_DECIMALS: u8 = TokenMetadata::MAX_DECIMALS;

    const DISTRIBUTE_PROC_NAME: &str = "timed_unlimited_fungible_faucet::distribute";
    const BURN_PROC_NAME: &str = "timed_unlimited_fungible_faucet::burn";

    // CONSTRUCTORS
    // --------------------------------------------------------------------------------------------

    /// Creates a new [`TimedUnlimitedFungibleFaucet`] component.
    ///
    /// The max supply is set to
    /// [`FungibleAsset::MAX_AMOUNT`](miden_protocol::asset::FungibleAsset::MAX_AMOUNT)
    /// since this faucet does not enforce supply limits at runtime, only time limits.
    ///
    /// # Errors
    ///
    /// Returns an error if:
    ...
    /// Creates a new [`TimedUnlimitedFungibleFaucet`] component from the given [`TokenMetadata`].
    ...
    /// Attempts to create a new [`TimedUnlimitedFungibleFaucet`] component from the associated
    ...
    ///   [`AccountComponentInterface::TimedUnlimitedFungibleFaucet`] component.
    /// - the decimals parameter exceeds maximum value of [`Self::MAX_DECIMALS`].
    ...
    fn try_from_interface(
        ...
        // Accept either TimedUnlimitedFungibleFaucet or TimedFungibleFaucet because the
        // compiled MASM procedures are identical (same distribute/burn logic). The auto-detection
        // in extract_standard_components will match TimedFungibleFaucet first, so we accept both.
        if !interface
            .components()
        ...
        let config_word: Word = storage
            .get_item(TimedUnlimitedFungibleFaucet::timed_config_slot())
            .map_err(|err| FungibleFaucetError::StorageLookupFailed {
                slot_name: TimedUnlimitedFungibleFaucet::timed_config_slot().clone(),
                source: err,
            })?;
        ...
        let owner_account_id_word: Word = storage
            .get_item(TimedUnlimitedFungibleFaucet::owner_config_slot())
            .map_err(|err| FungibleFaucetError::StorageLookupFailed {
                slot_name: TimedUnlimitedFungibleFaucet::owner_config_slot().clone(),
                source: err,
            })?;
    ...
    /// Returns the [`StorageSlotName`] where the [`TimedUnlimitedFungibleFaucet`]'s metadata is
    /// stored.
    ...
    /// Sets the token_supply (in base units) of the timed unlimited fungible faucet.
    ///
    /// # Errors
    ///
    /// Returns an error if:
...
impl From<TimedUnlimitedFungibleFaucet> for AccountComponent {
    fn from(faucet: TimedUnlimitedFungibleFaucet) -> Self {
        ...
        let config_slot = StorageSlot::with_value(
            TimedUnlimitedFungibleFaucet::timed_config_slot().clone(),
            Word::new(config_val),
        );
        ...
        let owner_slot = StorageSlot::with_value(
            TimedUnlimitedFungibleFaucet::owner_config_slot().clone(),
            owner_account_id_word,
        );

        let storage_schema = StorageSchema::new([
            TimedUnlimitedFungibleFaucet::metadata_slot_schema(),
            TimedUnlimitedFungibleFaucet::timed_config_slot_schema(),
            TimedUnlimitedFungibleFaucet::owner_config_slot_schema(),
        ])
        .expect("storage schema should be valid");

        let metadata = AccountComponentMetadata::new(TimedUnlimitedFungibleFaucet::NAME)
            .with_description(
                "Timed unlimited fungible faucet component for time-bounded minting and burning tokens without supply cap",
            )
        ...
        .expect("timed unlimited fungible faucet component should satisfy the requirements of a valid account component")
    }
}

impl TryFrom<Account> for TimedUnlimitedFungibleFaucet {
    type Error = FungibleFaucetError;

    fn try_from(account: Account) -> Result<Self, Self::Error> {
        let account_interface = AccountInterface::from_account(&account);

        TimedUnlimitedFungibleFaucet::try_from_interface(account_interface, account.storage())
    }
}
...
/// Creates a new faucet account with timed unlimited fungible faucet interface,
/// account storage type, owner account, and provided metadata (token symbol,
/// decimals, distribution end block).
///
/// The timed unlimited faucet interface exposes procedures:
/// - `distribute`, which mints assets and creates a note for the provided recipient within the
...
///
/// No supply cap is enforced at runtime. The `distribute` procedure can only be called
/// by the owner of the faucet within the distribution period. The `burn` procedure can
/// be called by anyone and requires the calling note to contain the asset to be burned.
pub fn create_timed_unlimited_fungible_faucet(
    init_seed: [u8; 32],
    symbol: TokenSymbol,
    decimals: u8,
    distribution_end: u32,
    ...
    let faucet_component =
        TimedUnlimitedFungibleFaucet::new(symbol, decimals, distribution_end, owner_account_id)?;
...
mod tests {
    ...
    #[test]
    fn timed_unlimited_faucet_contract_creation() {
        ...
        let token_symbol = TokenSymbol::try_from("TUF").unwrap();
        let decimals = 6u8;
        let distribution_end = 10_000u32;
        let storage_mode = AccountStorageMode::Private;

        let faucet_account = create_timed_unlimited_fungible_faucet(
            init_seed,
            token_symbol,
            decimals,
            distribution_end,
        ...
        assert_eq!(
            faucet_account
                .storage()
                .get_item(TimedUnlimitedFungibleFaucet::metadata_slot())
                .unwrap(),
            [
                Felt::ZERO,
        ...
        let faucet_component =
            TimedUnlimitedFungibleFaucet::try_from(faucet_account.clone()).unwrap();
    ...
    #[test]
    fn timed_unlimited_faucet_create_from_account() {
        let mock_word = Word::from([0, 1, 2, 3u32]);
        let mock_public_key = PublicKeyCommitment::from(mock_word);
        let mock_seed = mock_word.as_bytes();
        let owner_account_id = mock_owner_account_id();

        let token_symbol = TokenSymbol::new("TUF").expect("invalid token symbol");
        let faucet_account = AccountBuilder::new(mock_seed)
            .account_type(AccountType::FungibleFaucet)
            .with_component(
                TimedUnlimitedFungibleFaucet::new(token_symbol, 6, 10_000, owner_account_id)
                    .expect("failed to create a timed unlimited fungible faucet component"),
            )
            .with_auth_component(AuthFalcon512Rpo::new(mock_public_key))
            .build_existing()
            .expect("failed to create faucet account");

        let timed_unlimited_ff = TimedUnlimitedFungibleFaucet::try_from(faucet_account)
            .expect("timed unlimited fungible faucet creation failed");
        assert_eq!(timed_unlimited_ff.symbol(), token_symbol);
        assert_eq!(timed_unlimited_ff.decimals(), 6);
        assert_eq!(timed_unlimited_ff.max_supply(), Felt::new(FungibleAsset::MAX_AMOUNT));
        assert_eq!(timed_unlimited_ff.token_supply(), Felt::ZERO);
        assert_eq!(timed_unlimited_ff.distribution_end(), 10_000);
        assert_eq!(timed_unlimited_ff.owner_account_id(), owner_account_id);

        // invalid account: timed unlimited fungible faucet component is missing
    ...
    #[test]
    fn get_timed_unlimited_faucet_procedures() {
        let _distribute_digest = TimedUnlimitedFungibleFaucet::distribute_digest();
        let _burn_digest = TimedUnlimitedFungibleFaucet::burn_digest();
    }
}
