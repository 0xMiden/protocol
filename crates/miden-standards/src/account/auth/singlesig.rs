...
/// `miden::standards::auth::fee::pay_fee`: it creates a public BATCH_FEE note (see
/// [`BatchFeeNote`](crate::note::BatchFeeNote)) funded from the account's vault, so on
/// fee-charging chains the account must hold a sufficient balance of the payment asset. By
/// default the fee is paid in the native fee asset; the transaction's auth args can commit to
/// conversion info to pay in a different asset. On chains with a zero verification base fee no
/// note is created. The fee note is created before the transaction summary, so it is covered by
/// the signature.
///
