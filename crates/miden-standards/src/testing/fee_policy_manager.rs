...
impl FeePolicyManager {
    ...
    ///
    /// Its active policy is an empty [`BasicConstantFeePolicy`], which aborts fee estimation for
    /// any note script root; tests that consume notes need a policy with a fee schedule
    /// instead.
