...
impl P2idNote {
    ...
    /// - More than [`P2idNote::MAX_NUM_ASSETS`] assets were provided.
...
impl P2idNote {
    ...
    /// Maximum number of assets that a P2ID note may contain.
...
mod tests {
    ...
    fn builder_rejects_too_many_assets() {
        ...
        let err = P2idNote::builder()
            ...
            .assets(assets)
            .build()
            .expect_err("a note exceeding the P2ID asset limit must be rejected");

        assert_matches!(
            err,
            NoteError::TooManyAssetsForScript {
                num_assets,
                max_assets: P2idNote::MAX_NUM_ASSETS,
            } if num_assets == P2idNote::MAX_NUM_ASSETS + 1
        );
    }
}
