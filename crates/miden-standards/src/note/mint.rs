...
mod tests {
    ...
    #[test]
    fn create_rejects_faucet_id_not_matching_embedded_asset() {
        let faucet_a = dummy_faucet_id(1);
        let faucet_b = dummy_faucet_id(2);
        assert_ne!(faucet_a, faucet_b);

        // The asset is issued by faucet A, but the MINT note targets faucet B.
        let asset = FungibleAsset::new(faucet_a, 100).unwrap();
        let storage = MintNoteStorage::new_private(Word::default(), asset, Felt::ZERO);

        let mut rng = RandomCoin::new(Word::default());
        let result =
            MintNote::create(faucet_b, faucet_b, storage, NoteAttachments::default(), &mut rng);
