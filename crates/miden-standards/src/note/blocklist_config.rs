...
/// commitment, the authorized party is the note sender: the consuming account's `BlocklistManager`
...
/// `BlocklistManager` component whose blocklist is being managed. The `sender` is the account
...
/// `BlocklistManager` is installed on faucets, which are not necessarily network accounts, so an
...
impl BlocklistConfigNote {
    /// Builds a new [`BlocklistConfigNote`] that applies `config` to `account`.
    ///
    /// # Errors
    ///
