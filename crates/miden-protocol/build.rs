...
/// The batch kernel's advice-map sentinel keys, as `(constant name, domain message)`. Each key word
/// is four equal felts of the message's hash. The keys are arbitrary lookup sentinels (the lists'
/// integrity comes from binding every entry to a verified per-transaction note, not from the key);
/// deriving them from a domain message keeps them self-documenting rather than magic constants.
...
/// Derives the batch kernel's note-list keys by hashing their domain messages and emits the value
/// to both sides: a generated MASM module (`note_list_keys.masm`) the kernel pushes, and Rust
/// constants (`batch_kernel_constants.rs`) used to build the advice map. Hashing happens only here,
/// so the two sides agree by construction.
fn generate_batch_kernel_keys(asm_source_dir: &Path, build_dir: &str) -> Result<()> {
    let keys: Vec<(&str, u64)> = BATCH_KERNEL_KEY_MESSAGES
        .iter()
        .map(|(name, message)| (*name, EventId::from_name(message).as_felt().as_canonical_u64()))
        .collect();

    // MASM module imported by the prologue; each key word is `push.<value> dup dup dup`.
    let mut masm = String::from(
        "# This file is generated by build.rs, do not modify manually.\n\
         # Advice-map sentinel keys for the global note lists, derived from a domain message.\n\n",
    );
    for (name, value) in &keys {
        masm.push_str(&format!("pub const {name}={value}\n"));
    }
    ...
    for (name, value) in &keys {
        rust.push_str(&format!("pub const {name}: u64 = {value};\n"));
    }
