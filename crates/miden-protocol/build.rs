...
const BATCH_KERNEL_CONSTANTS_RS_FILE: &str = "batch_kernel_constants.rs";
...
fn main() -> Result<()> {
    ...
    // Derive the batch kernel's note-list keys (a generated MASM module + Rust constants) before
    // assembling, since the kernel imports the generated MASM module.
    generate_batch_kernel_keys(&source_dir, &build_dir)?;
...
// BATCH KERNEL KEYS GENERATION
// ================================================================================================

/// The batch kernel's advice-map sentinel keys, as `(constant name, domain message)`. Each key is
/// the poseidon2 hash of its message — a full word. The keys are arbitrary lookup sentinels (the
/// lists' integrity comes from binding every entry to a verified per-transaction note, not from the
/// key); deriving them from a domain message keeps them self-documenting rather than magic
/// constants.
const BATCH_KERNEL_KEY_MESSAGES: [(&str, &str); 2] = [
    ("INPUT_NOTE_LIST_KEY", "miden::batch_kernel::input_note_list"),
    ("OUTPUT_NOTE_LIST_KEY", "miden::batch_kernel::output_note_list"),
];

/// Derives the batch kernel's note-list keys by hashing their domain messages to a word and emits
/// the value to both sides: a generated MASM module (`note_list_keys.masm`) exposing a proc per key
/// that pushes the key word, and Rust constants (`batch_kernel_constants.rs`) used to build the
/// advice map. Hashing happens only here, so the two sides agree by construction.
fn generate_batch_kernel_keys(asm_source_dir: &Path, build_dir: &str) -> Result<()> {
    let keys: Vec<(&str, [u64; 4])> = BATCH_KERNEL_KEY_MESSAGES
        .iter()
        .map(|(name, message)| {
            let word = Poseidon2::hash(message.as_bytes());
            let felts: [u64; 4] = std::array::from_fn(|i| word.as_elements()[i].as_canonical_u64());
            (*name, felts)
        })
        .collect();

    // MASM module imported by the prologue. Each key is a proc that pushes its word so the prologue
    // can `exec` it directly.
    let mut masm = String::from(
        "# This file is generated by build.rs, do not modify manually.\n\
         # Advice-map sentinel keys for the global note lists, derived from a domain message hash.\n\n",
    );
    for (name, [f0, f1, f2, f3]) in &keys {
        let proc = name.to_lowercase();
        // Push the felts in reverse so the resulting stack word equals the `Word` the Rust advice
        // builder uses as the key (the last value pushed lands at stack position 0 / word element
        // 0).
        masm.push_str(&format!(
            "#! Pushes the `{name}` advice-map key word.\n\
             #!\n\
             #! Inputs:  []\n\
             #! Outputs: [{name}]\n\
             pub proc {proc}\n    push.{f3}.{f2}.{f1}.{f0}\nend\n\n",
    ...
    let masm_path = asm_source_dir
        .join(ASM_BATCH_KERNEL_DIR)
        .join("lib")
        .join("note_list_keys.masm");
    fs::write(masm_path, masm).into_diagnostic()?;

    // Rust constants used by `BatchKernel::build_advice_inputs`.
    let mut rust = String::from(
        "// This file is generated by build.rs, do not modify manually.\n\
         // Advice-map sentinel keys for the global note lists, derived from a domain message hash.\n\n",
    );
    for (name, [f0, f1, f2, f3]) in &keys {
        rust.push_str(&format!("pub const {name}: [u64; 4] = [{f0}, {f1}, {f2}, {f3}];\n"));
    }
    let rust_path = Path::new(build_dir).join(BATCH_KERNEL_CONSTANTS_RS_FILE);
    fs::write(rust_path, rust).into_diagnostic()
}
