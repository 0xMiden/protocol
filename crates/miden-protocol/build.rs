...
/// Reads the batch kernel MASM source from the `source_dir`, compiles it, and saves the result to
/// the `target_dir` as a `batch_kernel.masb` binary file.
///
...
/// `kernels/batch/main.masm` with the modules under `kernels/batch/lib/` statically linked under
/// the `$batch` namespace.
