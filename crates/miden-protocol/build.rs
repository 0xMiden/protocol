...
fn main() -> Result<()> {
    ...
    generate_batch_kernel_constants(&source_dir, &build_dir)?;
...
// BATCH KERNEL CONSTANTS FILE GENERATION
// ================================================================================================
...
fn generate_batch_kernel_constants(asm_source_dir: &Path, build_dir: &str) -> Result<()> {
    ...
    let regex =
        Regex::new(r"const\s+(?<name>\w+_LIST_KEY)\s*=\s*0x(?<value>[0-9A-Fa-f]+)").unwrap();

    // BTreeMap for deterministic ordering and to detect conflicting redefinitions.
    let mut constants: BTreeMap<String, u32> = BTreeMap::new();
    for entry in WalkDir::new(&batch_kernel_dir) {
        ...
        for capture in regex.captures_iter(&file_contents) {
            ...
            let value = u32::from_str_radix(&capture["value"], 16)
                .into_diagnostic()
                .wrap_err_with(|| format!("`const {name}` is not a valid u32 hex literal"))?;
            if let Some(existing) = constants.insert(name.clone(), value)
                && existing != value
            {
                return Err(miette::miette!(
                    "batch kernel constant `{name}` is defined with conflicting values"
                ));
            }
        }
    }
    ...
    let mut output = String::from(
        "// This file is generated by build.rs, do not modify manually.\n\
         // It mirrors the `*_LIST_KEY` advice-map keys defined in the batch kernel MASM.\n\n",
    );
    for (name, value) in &constants {
        output.push_str(&format!("pub const {name}: u32 = {value:#010x};\n"));
    }

    let output_path = Path::new(build_dir).join(BATCH_KERNEL_CONSTANTS_RS_FILE);
    fs::write(output_path, output).into_diagnostic()
}
