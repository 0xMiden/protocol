...
/// a validated symbol (same rules as [`crate::asset::TokenSymbol`] and
...
impl Symbol {
    ...
    /// Constructs a symbol up to 12 uppercase ASCII Latin letters (`A`–`Z`).
    ///
    /// # Errors
    /// Returns an error if:
    ...
    pub fn from_ascii_uppercase(s: &str) -> Result<Self, SymbolError> {
        let len = s.len();
    ...
    /// Constructs a symbol up to 12 characters from `A`–`Z` and `_`.
    ///
    /// # Errors
    /// Returns an error if:
    /// - The length of the provided string is less than 1 or greater than 12.
    /// - The string contains a character outside `A`–`Z` and `_`.
    pub fn from_ascii_uppercase_and_underscore(s: &str) -> Result<Self, SymbolError> {
        let len = s.len();
        if len == 0 || len > Self::MAX_SYMBOL_LENGTH {
            return Err(SymbolError::InvalidLength(len));
        }
...
mod tests {
    ...
    fn symbol_encode_decode_roundtrip() {
        let symbol = Symbol::from_ascii_uppercase("MIDEN").unwrap();
    ...
    fn symbol_rejects_invalid_values() {
        assert_matches!(
            Symbol::from_ascii_uppercase("").unwrap_err(),
            SymbolError::InvalidLength(0)
        );
        assert_matches!(
            Symbol::from_ascii_uppercase("ABCDEFGHIJKLM").unwrap_err(),
            SymbolError::InvalidLength(13)
        );
        assert_matches!(
            Symbol::from_ascii_uppercase("A_B").unwrap_err(),
            SymbolError::InvalidCharacter
        );

        assert_matches!(
            Symbol::from_ascii_uppercase_and_underscore("MINTER-ADMIN").unwrap_err(),
