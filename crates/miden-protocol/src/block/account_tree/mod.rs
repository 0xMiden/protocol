...
pub(super) mod tests {
    ...
    pub(crate) fn setup_duplicate_prefix_ids() -> [(AccountId, Word); 2] {
        let id0 = AccountId::try_from(account_id(AccountStorageMode::Public, 0xaabb_ccdd)).unwrap();
        let id1 = AccountId::try_from(account_id(AccountStorageMode::Public, 0xaabb_ccff)).unwrap();
