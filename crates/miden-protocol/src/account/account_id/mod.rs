...
///
/// ```text
/// prefix: [hash (59 bits) | account type (1 bit) | version (4 bits)]
...
/// This process is repeated until the hash's first element has the desired account type and
/// version and the suffix' most significant bit is zero.
///
...
/// hash.
///
/// In total, due to requiring specific bits for account type, version and the most significant bit
...
impl AccountId {
    ...
    pub fn dummy(
        ...
        match version {
            AccountIdVersion::Version1 => Self::V1(AccountIdV1::dummy(bytes, account_type)),
        }
    }

    /// Grinds an account seed until its hash matches the given `account_type` and `version` and
    /// returns it as a [`Word`]. The input to the hash function next to the seed are the
    /// `code_commitment` and `storage_commitment`.
    ///
