...
/// This process is repeated until the hash's first element has the desired storage mode and
...
///   number of storage mode bits will not cause the version to be at a different offset. This is
...
/// - The version and storage mode are part of the prefix which is included in the representation of
...
impl AccountId {
    ...
    /// Constructs an [`AccountId`] for testing purposes with the given storage mode.
    ///
    ...
    /// - The least significant byte of the prefix is set to the given version and storage mode.
    ...
    /// Grinds an account seed until its hash matches the given `storage_mode` and `version` and
