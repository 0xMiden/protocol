...
mod asset_callback_flag;
pub use asset_callback_flag::AssetCallbackFlag;
...
///
/// ```text
/// prefix: [hash (58 bits) | asset callback flag (1 bit) | account type (1 bit) | version (4 bits)]
...
/// This process is repeated until the hash's first element has the desired account type, asset
/// callback flag and version and the suffix' most significant bit is zero.
///
...
/// hash.
///
/// In total, due to requiring specific bits for account type, asset callback flag, version and the
/// most significant bit in the suffix, generating an ID requires 7 bits of Proof-of-Work.
///
/// # Constraints
///
...
///   number of account type bits will not cause the version to be at a different offset. This is
///   important so that a parser can always reliably read the version and then parse the remainder
...
impl AccountId {
    ...
    /// Constructs an [`AccountId`] for testing purposes with the given account type.
    ///
    ...
    /// - The least significant byte of the prefix is set to the given version and account type.
    ...
    pub fn dummy(
        ...
        match version {
            AccountIdVersion::Version1 => {
                Self::V1(AccountIdV1::dummy(bytes, account_type, asset_callbacks))
            },
        }
    }

    /// Grinds an account seed until its hash matches the given `account_type`, `asset_callbacks`
    /// and `version` and returns it as a [`Word`]. The input to the hash function next to the
    /// seed are the `code_commitment` and `storage_commitment`.
    ///
    ...
    /// Returns the [`AssetCallbackFlag`] of this account ID.
    pub fn asset_callback_flag(&self) -> AssetCallbackFlag {
        match self {
            AccountId::V1(account_id) => account_id.asset_callback_flag(),
        }
    }
...
mod tests {
    ...
    use super::*;
    use crate::account::account_id::v1::{extract_account_type, extract_version};
    ...
    fn bech32_encode_decode_roundtrip() -> anyhow::Result<()> {
        ...
        for network_id in [
            ...
            .into_iter()
            {
                ...
                assert_eq!(
                    account_id.asset_callback_flag(),
                    decoded_account_id.asset_callback_flag(),
                    "asset callback flag failed for {account_id}"
                );
                ...
                assert_eq!(extract_account_type(data[8] as u64), account_id.account_type());
            }
        }
